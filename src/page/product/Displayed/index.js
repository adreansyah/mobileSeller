...
import BtnAddHidden from "../btnAddHidden";
...
const Displayed = props => {
  ...
  const handleSelectChange = event => {
    ...
    setSelected([...arr]);
    const data = product.displayed.data.map(list => list.productId.toString());;
...
const ProductBlock = props => {
  const {product,RequestProductDisplay,setSelectedAll,datatemp,selectedProps} = props;
  ...
  const [ishiddenData,setIsHiddenData] = useState([]);
  const [selected,setSelected] = useState([]);
  ...
  const handleSelectChange = event => {
    ...
    setSelected([...arr]);
    if(product.displayed.data.length === arr.length) {
  ...
  const fetchPerPage = () => {
    const statCode = 103,
      page = props.product.displayed.page + 1,
      size = 5,
      loadmore = "isloadmore"
    RequestProductDisplay(statCode,page,size,null,loadmore);
    setSelectedAll([]);
  };
  ...
  const handleHideOpen = (data) => {
    setIsHideOpen(true);
    setIsContextOpen(false);
    setIsHiddenData([data !== undefined ? data.toString() : []])
  };
  ...
  return (
    <Container>
      <InfiniteScroll
        ...
        hasMore={product.displayed.loadMore}
        isLoading={product.displayed.isLoading}
        loader={
          <Segment
            key={0}
            width="100%"
        ...
        error={
          product.displayed.data.length === 0 &&
          !product.displayed.loadMore && (
            <ErrorBlock
              key={1}
              errOnSearch={product.displayed.query ? true : false}
            />
          )
        }
      >
        {product.displayed.data.map((item,index) => {
          return (
      ...
      {
        selectedProps.length === 0 && selected.length < 1 ? <BtnAddProduct {...props} /> : <BtnAddHidden handleHideOpen={() => handleHideOpen()} />
      }
      <StockTray {...props} isUpdate="displayed" productId={isProductId} stockItem={stockItem} isOpen={isStockOpen} onClose={() => setIsStockOpen(false)} />
      <HideTray {...props} isShould="hidden" setSelectedAll={() => setSelectedAll([])} selected={selected} ishiddenData={ishiddenData} isOpen={isHideOpen} onClose={() => setIsHideOpen(false)} />
    </Container >
  );
};

...
const ProductBlock = props => {
  const {product,RequestProductSoldOut} = props;
  ...
  const fetchPerPage = () => {
    const statCode = 103,
      page = props.product.soldout.page + 1,
      size = 5,
      loadmore = "isloadmore"
    RequestProductSoldOut(statCode,page,size,null,loadmore);
  };
  ...
  return (
    <Container>
      <InfiniteScroll
        refs={"soldout-product"}
        loadMore={fetchPerPage}
        hasMore={product.soldout.loadMore}
        isLoading={product.soldout.isLoading}
        loader={
          <Segment
            key={0}
            width="100%"
        ...
        error={
          product.soldout.data.length === 0 &&
          !product.soldout.loadMore && (
            <ErrorBlock
              key={1}
              errOnSearch={product.soldout.query ? true : false}
            />
          )
        }
      >
        {product.soldout.data.map((item,index) => {
          return (
      ...
      <BtnAddProduct {...props} />
      <StockTray {...props} isUpdate="soldout" productId={isProductId} stockItem={stockItem} isOpen={isStockOpen} onClose={() => setIsStockOpen(false)} />
    </Container >
  );
};

...
import { getOrderPurchased, setOrderPurchased, getOrderCount } from 'store/actions/order'
...
class PurchasedOrder extends Component {
    constructor(props) {
        super(props)
        this.state = {
            params: this.params,
    ...
    componentDidMount() {
        const { params } = this.props.order.purchased
    ...
    fetchPerPage = () => {
        const { getOrderPurchased, order } = this.props
        const { params } = this.state
        const page = order.purchased.params ? order.purchased.params.page + 1 : 0
        this.setState({ params: { ...params, page } }, () => getOrderPurchased(this.state.params))
    }
    handleFilter = async nextParams => {
        const { setOrderPurchased, getOrderPurchased, getOrderCount } = this.props
        const isFilterDiff = this.diffObject(this.params, nextParams)
        this.setState({ params: nextParams, isFilterDiff })
        await setOrderPurchased({ data: [] })
        await getOrderPurchased(nextParams)
        await getOrderCount(nextParams)
    }
    handleSort = async nextParams => {
        const { setOrderPurchased, getOrderPurchased } = this.props
    ...
    handleShowAll = (orderIdx, OrderListIdx) => {
        const { order, setOrderPurchased } = this.props
        const orderData = order.purchased.data
        orderData[orderIdx].orderList[OrderListIdx].show = !orderData[orderIdx].orderList[
            OrderListIdx
        ].show
        setOrderPurchased({ data: [...orderData] })
    }
    render() {
        ...
        return (
            <>
                ...
                <Container className="page-order" pt={12}>
                    <InfiniteScroll
                        refs="purchased-order-scroll"
                        loadMore={() => this.fetchPerPage()}
                        hasMore={order.purchased.loadMore}
                        isLoading={!order.purchased.isLoaded}
                        loader={
                            <Segment
                                key={0}
                                width="100%"
                        ...
                        error={
                            order.purchased.data.length === 0 &&
                            order.purchased.isLoaded && (
                                <ErrorBlock
                                    key={1}
                    ...
                    >
                        {order.purchased.data.map((order, orderIndex) => (
...
export default connect(mapStateToProps, mapDispatchToProps)(PurchasedOrder)
